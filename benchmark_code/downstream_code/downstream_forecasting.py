...
class TransformerForecaster(torch.nn.Module):
    def __init__(
        ...
        self.decoder_embedding = nn.Linear(n_out_features, d_model)
    ...
    def forward(self, X, forecasting_X):
        ...
        X_embedding = self.pos_encoding(self.encoder_embedding(X))
        encoding, _ = self.transformer_encoder(X_embedding)

        if self.training:
            X_pred = forecasting_X
        else:
            X_pred = torch.zeros(
                X.shape[0],
                self.n_out_steps,
        ...
        X_pred_embedding = self.pos_encoding(self.decoder_embedding(X_pred))
        decoding, _, _ = self.transformer_decoder(X_pred_embedding, encoding)
    ...
    for epoch in range(n_epochs):
        ...
        for idx, data in enumerate(train_dataloader):
            ...
            if "RNN" in model._get_name():
                ...
                predictions = model(X, y[:, -n_forecasting_steps:])
